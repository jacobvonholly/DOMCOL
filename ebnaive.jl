...
function BetterHeaps()
    ...
    while remaininglength >= Heapparam && time() - starttime <= 10 && res <= strikes
        ...
        #println("test")
        res += naivemain(L[iteration])
        iteration += 1
    end
    push!(L, [0,[TempList]])
    if time() - starttime < 10 && remaininglength < Heapparam
        naivemain(L[length(L)])
    end
...
function naivemain(L)
    bf = length(dom_pairs)
    hlen = length(L[2])
    i = 1
    start = time()
    while i < hlen && time() - start < 10
        j = i + 1
        while j <= hlen
            compare(L[2][i],L[2][j])
            j += 1
...
while directoryindex <= directorylength
    ...
    a = 1000
    open("0000resultsmy2n.txt","a") do io
